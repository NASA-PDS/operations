...
def do_git_stuff():
    """Move the new sitemap to the repo. Update timestamp of local, partial sitemap. (In that order.)
    Then do git stuff.
    """
    ...
    if not repo.is_dirty():
        _logger.info('The %s repo is currently clean, so no updates to commit', repo_path)
        return
    ...
    if 'sitemap.xml' not in changed:
        _logger.info('The sitemap.xml hasn\'t been updated this time, so no changes to commit')
        return

    # Can't use the `add` API provided by GitPython since it doesn't support the version of the
    # repository in /data/www/pds/htdocs; but it does provide a "git" object that calls
    # `git` directly
...
def recreate_partial_sitemap(recreate_callback):
    ...
    Sitemap is partial in that it has existing ds-view URLs removed. Closing `urlset` tag is also removed.

    :param recreate_callback: after this function is complete, calls `write_parsed_response` to append the new ds-view URLs
    """
    # Subprocess-free "copy"?
    shutil.copy(target, os.getcwd())

    with open('sitemap.xml', 'r') as reader:
        # there are already some ds-view results in the sitemap (don't know where they came from)
        # e.g. https://pds.nasa.gov/ds-view/pds/viewCollection.jsp?identifier=urn%3Anasa%3Apds%3Amaven.lpw.raw%3Adata.act&amp;version=2.5
        # the URL specifies an older version while the page reflects the (supposably) most recent version
        # additionally, the query returns the (supposably) most recent version
        # --> just delete these from the sitemap. the query results have the latest.
        ds_view_item = False
...
def get_file_modification_time(filepath):
    ...
    :param filepath:
    :return: seconds between epoch and file modification time. bigger number = more recently modified. float.
    """
...
def was_repo_sitemap_modified():
    """Compare modification timestamps of local, partial sitemap to repository's full sitemap.
    If repository's sitemap timestamp is longer (more recent), it has been modified since this script was last run and
    the local, partial sitemap must be recreated.

    :return: True if repository sitemap has been modified since this script was last run. False if otherwise. boolean
    """
    repo_sitemap_modification_time = get_file_modification_time(target)
    local_sitemap_modification_time = get_file_modification_time('sitemap-partial.xml')

    if repo_sitemap_modification_time < local_sitemap_modification_time:
        _logger.info('Repository\'s sitemap hasn\'t changed since last run. Continuing from the existing local, partial sitemap.')
        return False
    else:
        _logger.info('Repository\'s sitemap has changed since last run. Recreating the local, partial sitemap.')
        return True


def prepare_to_write_parsed_response(write_callback):
    """Check if repository's sitemap has been modified since this script was last run.
    If yes, recreate the local, partial sitemap before writing the parsed response to create the full sitemap.
    If no, proceed directly to writing the parsed response to create the full sitemap.

    :param write_callback: after this function is complete, calls `do_git_stuff` for last steps
    """
...
def get_most_recent_lastmod_date(list_of_lastmod_dates):
    """Find most recent modification_date from list of string dates from a single response doc
    ...
    for lastmod_date in list_of_lastmod_dates:
        # max() appears to work with the dates in string form. Still, convert to datetime.date just in case
...
def parse_response(list_of_docs):
    """Parse list of json objects from the query's response.
    Look for `resLocation` and most recent `modification_date`, which can have multiple dates.
    Ignore values of `resLocation` that are not from PDS site,
    ...
    for doc in list_of_docs:
        # ignoring URLs not from PDS, e.g.
        # https://archives.esac.esa.int/psa/pdap/metadata?DATA_SET_ID=MEX-M-MRS-1/2/3-PRM-0430-V1.0&RETURN_TYPE=HTML
        if doc['resLocation'].startswith('/ds-view'):
            ...
            if 'modification_date' not in doc:
                ...
                if len(doc['modification_date']) == 1:
                    parsed_response[location] = convert_string_to_date(doc['modification_date'][0])
                else:
                    parsed_response[location] = get_most_recent_lastmod_date(doc['modification_date'])


def set_total_number_of_docs(number):
    """Set global variable, `total_number_of_docs`, which determines when to stop querying.
    Also, overwrite this variable inside previous_run.py to reference for the next script run.
...
def run_script_after_x_minutes(x):
    """Wait x number of minutes before running the script again.
    Used in the case of a requests exception or if the data is being re-indexed.

    :param x: number of minutes to wait before running the script again. integer.
    """
    time.sleep(60 * x)
...
def compare_totals(current_total):
    """Compare the total number of results (aka docs) between the current script run and the last time the script was
    run. If the current total is less than the previous total, this is an indicator that the data is being re-indexed.
...
def set_and_get_query_parameters(start_number):
    ...
    return {
        'wt': 'json',
        # doesn't work  # 'q': 'product_class:Product_Collection%20OR%20product_class:Product_Bundle%20OR%20product_class:Product_Data_Set_PDS3%20OR%20product_class:Product_Document',
        'q': 'product_class:Product_Collection OR product_class:Product_Bundle OR product_class:Product_Data_Set_PDS3 OR product_class:Product_Document',
...
def do_query_with(start_number, query_callback):
    ...
    :param start_number: for `start` parameter of query. integer
    :param query_callback: after this function is complete, calls `parse_response` with the query's response
    """
    ...
    try:
        ...
        json_response = requests.get(search_url, params=query_parameters).json()['response']
    ...
    # should happen only with very first query. Am not tying it to "start=0" in case we want that to be variable
...
def check_if_more_queries(new_start_parameter_value):
    """Check against global variable, `total_number_of_docs`, for more queries to run.
    If yes (there's more), run them. If no, write the parsed responses to XML format.

    :param new_start_parameter_value: the already-used `start` parameter plus the programmer-defined increment. integer
    """
    if new_start_parameter_value < total_number_of_docs:
        do_query_and_parse_response(new_start_parameter_value, check_if_more_queries)
    else:
...
def do_query_and_parse_response(start_parameter_value, looped_callback):
    ...
    :param looped_callback: after this function is complete, calls `check_if_more_queries`, which calls this if needed
    """
...
if __name__ == '__main__':
    ...
    _logger.info(f'=== Starting update of sitemap.xml with ds-view pages on {datetime.now().strftime("%a %b %d, %Y at %H:%M:%S")}')
    do_query_and_parse_response(0, check_if_more_queries)
